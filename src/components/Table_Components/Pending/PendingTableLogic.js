...
function PendingTableLogic({ pendingDB }) {
  ...
  function forStuckState() {
    ...
    if(stuckState !== "OG"){
      ...
      for (let txn of DB) {
        if(txn.statesEntered1.substring(txn.statesEntered1.length - 2) == stuckState){
          DB1.push(txn)
        }
      }
    }
    ...
    <>
      <div className="filter">   
        ...
        <span>
          ...
          <input className="date1" placeholder="Time(Hrs)" value={time} onChange={(e) => setTime(e.target.value)}  style={{ width: '100px' }}/>
        </span>
...
export default PendingTableLogic;
